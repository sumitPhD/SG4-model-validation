...
T = SGP_test_case_launch1(1,:); % minutes
x = SGP_test_case_launch1(2:4,:);
N = length(x);
LLA_test_case_launch1 = zeros(4,N);
...
for i =1:N % keeping the gap high as the (it was taking too much time , not needed that much accuracy)
    ...
    LLA_test_case_launch1(1,i) = T(i);
    LLA_test_case_launch1(2:4,i) = [LAT; LONG; ALT];
% plot(LLA_test_case(3,i),LLA_test_case(2,:))
% hold on
end
save LLA_test_case_launch1
plot(LLA_test_case_launch1(3,:),LLA_test_case_launch1(2,:))
xlabel('Longitude');
ylabel('Lattitude');
